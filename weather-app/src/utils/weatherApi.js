...
const polishCityNames = {
  ...
  krakow: { pl: "Kraków", coords: { lat: 50.0647, lon: 19.945 } },
  gdansk: { pl: "Gdańsk", coords: { lat: 54.352, lon: 18.6466 } },
...
export const fetchWeatherData = async (cityName) => {
  try {
    ...
    console.warn(
      `Błąd API dla ${cityName}, używam danych testowych:`,
      error.message
    );
...
export const fetchForecast = async (cityName) => {
  try {
    ...
    console.warn(
      "Bląd pobierania prognozy, używam danych testowych:",
      error.message
    );
...
export const fetchAllCitiesWeather = async (cities = defaultCities) => {
  ...
  results.forEach((r, i) => {
    if (r.status === "fulfilled") {
      ...
      console.warn(
        `fetchAllCitiesWeather: failed to load ${cities[i]}:`,
        r.reason
      );
    }
  });
